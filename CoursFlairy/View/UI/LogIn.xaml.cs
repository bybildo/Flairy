...
namespace CoursFlairy.View.UI
{
    ...
    public partial class LogIn : UserControl
    {
        ...
        private void LoginProcces()
        {
            ...
            string query = $"SELECT TOP 1 ID FROM [dbo].[Airline] WHERE Email LIKE @login AND PasswordHash = @password";
            ...
            query = $"SELECT TOP 1 ID FROM [dbo].[User] WHERE {forquery} LIKE @login AND PasswordHash = @password";
            using (SqlCommand command = new SqlCommand(query, DataBase.GetConnection()))
            {
                command.Parameters.AddWithValue("@login", login.Text);
                command.Parameters.AddWithValue("@password", GetPasswordHash(password.Password));
